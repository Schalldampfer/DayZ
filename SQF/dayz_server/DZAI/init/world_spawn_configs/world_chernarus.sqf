...
_DZAI_spawn = {
	// [type], [minimum AI, max additional AI], level, spawn chance
	{
		private ["_mark","_text","_name"];
		_text = text _x;
		if ((_text != "") && ((random 1) < (_this select 3))) then {
			_name = format["DZAI_%1",_forEachIndex];
			_mark = createMarkerLocal [_name, locationPosition _x];
			...
			_mark setMarkerSizeLocal [200, 200];
			[_name,(_this select 1),[],(_this select 2)] call DZAI_static_spawn;
			diag_log format["[DZAI] %3:%1:%2",_text,_this,_name];
		};
